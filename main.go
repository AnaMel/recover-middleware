package main

import (
	"fmt"
	"log"
	"net/http"
	"runtime/debug"
)

func main() {
	mux := http.NewServeMux()
	mux.HandleFunc("/panic/", panicDemo)
	mux.HandleFunc("/panic-after/", panicAfterDemo)
	mux.HandleFunc("/", hello)
	log.Fatal(http.ListenAndServe(":3000", recoverMw(mux, true)))
}

func recoverMw(mux http.Handler, isDevEnv bool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		defer func() {
			if err := recover(); err != nil {
				log.Println(err)

				// extract and print stack trace
				stack := debug.Stack()
				log.Println(string(stack))

				if !isDevEnv {
					http.Error(w, "Something went wrong:", http.StatusInternalServerError)
					return
				}

				w.WriteHeader(http.StatusInternalServerError)
				fmt.Fprintf(w, "<h1>panic: %v</h1><pre>%s</pre>", err, stack)
			}
		}()

		cw := &customWriter{ResponseWriter: w}
		mux.ServeHTTP(cw, r)
		cw.flush()
	}
}

type customWriter struct {
	http.ResponseWriter
	writes [][]byte
	status int
}

func (cw *customWriter) Write(b []byte) (int, error) {
	cw.writes = append(cw.writes, b)
	return len(b), nil

}

func (cw *customWriter) WriteHeader(statusCode int) {
	cw.status = statusCode
}

func (cw *customWriter) flush() error {
	if cw.status != 0 {
		cw.ResponseWriter.WriteHeader(cw.status)
	}

	for _, write := range cw.writes {
		_, err := cw.ResponseWriter.Write(write)
		if err != nil {
			return err
		}
	}
	return nil
}

func panicDemo(w http.ResponseWriter, r *http.Request) {
	funcThatPanics()
}

func panicAfterDemo(w http.ResponseWriter, r *http.Request) {
	fmt.Fprint(w, "<h1>Hello!</h1>")
	funcThatPanics()
}

func funcThatPanics() {
	panic("Oh no!")
}

func hello(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintln(w, "<h1>Hello!</h1>")
}
